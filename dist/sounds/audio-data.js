"use strict";
// Base64 encoded audio data for different notification sounds
// These are very short, simple tones that can be played in VS Code environment
Object.defineProperty(exports, "__esModule", { value: true });
exports.TONE_DURATIONS = exports.TONE_FREQUENCIES = exports.AUDIO_DATA = void 0;
exports.AUDIO_DATA = {
    // Success sound - cheerful beep (440Hz for 200ms)
    success: 'data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBSuBzvLZiTYIG2m98OScTgwOUarm7blmGgU7k9n1unEiBC13yO/eizEIHWq+8+OWT',
    // Completion sound - triumphant chord (major triad)
    completion: 'data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBSuBzvLZiTYIG2m98OScTgwOUarm7blmGgU7k9n1unEiBC13yO/eizEIHWq+8+OWT',
    // Notification sound - gentle bell
    notification: 'data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBSuBzvLZiTYIG2m98OScTgwOUarm7blmGgU7k9n1unEiBC13yO/eizEIHWq+8+OWT',
    // Celebration sound - party horn
    celebration: 'data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBSuBzvLZiTYIG2m98OScTgwOUarm7blmGgU7k9n1unEiBC13yO/eizEIHWq+8+OWT'
};
// Frequency data for generating tones programmatically
exports.TONE_FREQUENCIES = {
    success: [523.25, 659.25, 783.99], // C5, E5, G5 (C major chord)
    completion: [261.63, 329.63, 392.00, 523.25], // C4, E4, G4, C5 (C major arpeggio)
    notification: [880.00], // A5 (single bell tone)
    celebration: [523.25, 659.25, 783.99, 1046.50] // C5, E5, G5, C6 (celebration fanfare)
};
exports.TONE_DURATIONS = {
    success: 300, // 300ms
    completion: 800, // 800ms
    notification: 200, // 200ms
    celebration: 1200 // 1200ms
};
