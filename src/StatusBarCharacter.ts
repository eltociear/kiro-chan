...
export class StatusBarCharacter implements IStatusBarCharacter {
  ...
  constructor() {
    ...
    this.stateAnimationBridge = new StateAnimationBridge(
      this.stateMonitor,
  ...
  private attachToStatusBar(): void {
    ...
    if (position === "left") {
      this.statusBarContainer.insertBefore(
        this.element,
        this.statusBarContainer.firstChild
      );
    } else {
  ...
  private findStatusBarContainer(): HTMLElement | null {
    try {
      ...
      console.warn(
        "[StatusBarCharacter] Status bar not found, creating fallback container"
      );
  ...
  private initializeComponents(): void {
    ...
    this.animationController.setAnimationSpeed(
  ...
  showSettings(): void {
    ...
    import("./ui/SettingsUI")
      ...
      .catch((error) => {
        console.error(
          "[StatusBarCharacter] Failed to load settings UI:",
          error
        );
      });
  }
  ...
  private setupErrorHandling(): void {
    ...
    if (typeof window !== "undefined") {
      ...
      window.addEventListener(
        "kiro-character:reduce-animation-complexity",
        () => {
  ...
  private enableMinimalMode(): void {
    try {
      ...
      console.error(
        "[StatusBarCharacter] Failed to enable minimal mode:",
        error
      );
    }
  }
  ...
  private fallbackToStaticDisplay(): void {
    try {
      ...
      console.error(
        "[StatusBarCharacter] Failed to fallback to static display:",
        error
      );
    }
  }
  ...
  private reduceAnimationComplexity(): void {
    try {
      ...
      console.error(
        "[StatusBarCharacter] Failed to reduce animation complexity:",
        error
      );
    }
  }
  ...
  private useDefaultSettings(): void {
    try {
      ...
      console.error(
        "[StatusBarCharacter] Failed to use default settings:",
        error
      );
    }
  }
  ...
  private enableFallbackStateDetection(): void {
    try {
      ...
      console.error(
        "[StatusBarCharacter] Failed to enable fallback state detection:",
        error
      );
    }
  }
