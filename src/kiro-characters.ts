...
export function getKiroCharacter(state: string = 'normal'): string {
    // For now, use the purple square as it's closest to the SVG
    switch (state) {
        case 'idle':
            return KIRO_CHARS.purple;
        case 'active':
            return KIRO_CHARS.faces.active;
        case 'error':
            return KIRO_CHARS.faces.error;
        case 'complete':
            return KIRO_CHARS.faces.complete;
        default:
            return KIRO_CHARS.purple;
    }
}