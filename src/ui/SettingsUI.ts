...
export class SettingsUI {
  ...
  private statusBarCharacter: StatusBarCharacter | null;
  ...
  show(): void {
    try {
      ...
      ErrorHandler.handleDomError(error as Error, {
  ...
  private createForm(): HTMLElement {
    ...
    // Enabled toggle
    const enabledGroup = this.createFormGroup(
      'enabled',
      'Enable Character',
      'checkbox',
    ...
    const speedGroup = this.createFormGroup(
      'animationSpeed',
      'Animation Speed',
      'range',
      this.settingsManager.getAnimationSpeed(),
  ...
  private createPositionGroup(): HTMLElement {
    const group = document.createElement('div');
    group.className = 'kiro-settings-group';
  ...
  private createPreviewGroup(): HTMLElement {
    const group = document.createElement('div');
  ...
  private addFormEventListeners(form: HTMLElement): void {
    // Real-time preview updates
    const enabledInput = form.querySelector('#enabled') as HTMLInputElement;
    const speedInput = form.querySelector('#animationSpeed') as HTMLInputElement;
    const positionInputs = form.querySelectorAll('input[name="position"]') as NodeListOf<HTMLInputElement>;
  ...
  private updatePreview(): void {
    ...
    const form = this.modal.querySelector('.kiro-settings-form') as HTMLFormElement;
    const preview = this.modal.querySelector('.kiro-settings-preview .kiro-character') as HTMLElement;
    
    if (!form || !preview) return;

    const enabled = (form.querySelector('#enabled') as HTMLInputElement)?.checked;
    const speed = parseFloat((form.querySelector('#animationSpeed') as HTMLInputElement)?.value || '1');
  ...
  private async saveSettings(): Promise<void> {
    try {
      ...
      // Get form values
      const enabled = (form.querySelector('#enabled') as HTMLInputElement)?.checked ?? true;
      const speed = parseFloat((form.querySelector('#animationSpeed') as HTMLInputElement)?.value || '1');
      const positionInput = form.querySelector('input[name="position"]:checked') as HTMLInputElement;
      const position = (positionInput?.value as 'left' | 'right') || 'right';
      ...
      ErrorHandler.handleSettingsError(error as Error, {
        action: 'saveSettings'
      });
  ...
  private resetToDefaults(): void {
    try {
      ...
      (form.querySelector('#enabled') as HTMLInputElement).checked = true;
      (form.querySelector('#animationSpeed') as HTMLInputElement).value = '1.0';
      (form.querySelector('#position-right') as HTMLInputElement).checked = true;
